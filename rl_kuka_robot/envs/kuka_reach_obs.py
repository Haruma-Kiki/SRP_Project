...
class kukaReachGymEnvblock(kukaGymEnv):
    def __init__(self, urdfRoot=robot_data.getDataPath(),
                 ...
                 includeVelObs=True,
                 reward_type=1):
        init_joints = [0.3,-0.4,1.5,0,0,0]
        super().__init__(urdfRoot, useIK, isDiscrete, actionRepeat, renders,
                         maxSteps, numControlledJoints, fixedPositionObj, init_joints=init_joints)
    ...
    def reset(self):
        ...
        _obstacle = p.loadURDF(os.path.join(self._urdfRoot, "kuka_kr6_support/block.urdf"), basePosition=[0.5,0,0.75],useFixedBase=True)
        self.obstacles.extend([0.5,0,0.15])
    ...
    def _sample_pose(self):
        ...
        ws_lim = [[0.2,0.6],[0.05,0.4],[0,0.2]]
        px1 = np.random.uniform(
            low=ws_lim[0][0], high=ws_lim[0][1])
        py1 = np.random.uniform(
            low=ws_lim[1][0], high=ws_lim[1][1])
